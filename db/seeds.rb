...
office_listing_data = File.open('office_data.txt', 'r')
office_listing_images = File.open('office_image_data.txt', 'r')

broker_1 = Broker.create(email: 'leejhampton@gmail.com', password: 'password123', password_confirmation: 'password123', first_name: 'Lee', last_name: 'Hampton')
...
office_listing_data.each_line do |line|
  ...
  OfficeListing.create(address: address, office_type: space_type, size: size, 
                       rent: rent, details: "This is an office", term_length: '5 years', 
                       neighborhood_id: hood_hash[neighborhood].id, broker_id: broker_1.id)
end

office_data_file.write address + "*" + unit + "*" + neighborhood + "*" + space_type + "*" + rent_type + "*" + size + "*" + price + '\n'

>>>>>>> c0508d63aa8b92f7e8b028f37546a5cf3ce44a19
